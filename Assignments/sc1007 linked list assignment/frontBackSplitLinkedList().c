...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	int frontindex=0, backindex=0, size, parity=1; // 0 for even, 1 for odd
	// checks if ll has odd or even number of elements
	if (ll->size%2==0) parity=0;
	// odd number of elements, put last element in frontlist
	if (parity==1) 
	{
		for (size = (ll->size/2)+1; size > 0; size--) {
			insertNode(resultFrontList, frontindex, ll->head->item);
			removeNode(ll, 0);
			frontindex++;
		}
		while (ll->size != 0)
		{
			insertNode(resultBackList, backindex, ll->head->item);
			removeNode(ll, 0);
			backindex++;
		}
	}
	
	// even number of elements, put rest of elements into backlist
	if (parity==0) {
		for (size = ll->size/2; size > 0; size--) {
			insertNode(resultFrontList, frontindex, ll->head->item);
			removeNode(ll, 0);
			frontindex++;
		}
