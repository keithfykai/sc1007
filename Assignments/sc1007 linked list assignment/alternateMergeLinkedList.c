...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	ListNode *ll2ptr;
	int ll1index=0;	
	if (ll1->size>=ll2->size) // can fit all elements from ll2 into ll1
	{
		while (ll2->size!=0) {
			insertNode(ll1, ll1index+1, ll2->head->item);
			removeNode(ll2, 0);
			ll1index += 2; // increments to 2 elements after
		}
		return;
	}

	if (ll1->size<ll2->size) // cannot fit all elements from ll2 into ll1, limiting factor is ll1.
	{
