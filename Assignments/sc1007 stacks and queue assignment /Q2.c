...
void removeEvenValues(Stack *s)
{
	...
	while (isEmptyStack(s)!=1) {
		temp = pop(s); // pops out element to check and stores in a temp variable
		// even number need pop out and store in odd stack if not continue
		if (temp%2 == 1) push(&odd,temp);
	}
	while (isEmptyStack(&odd)!=1) {
		temp = pop(&odd);
		push(s,temp);
	}
}
